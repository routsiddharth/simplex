...
| **catalog** | 5 min | Tracked series → the active (subscribed) market set. |
| **websocket** | persistent | Orderbook / trade / lifecycle deltas → `raw_events`. |
| **snapshot** | 10 s | Replays `raw_events` → the `snapshots` grid + book checkpoints. |
...
`cd ingest && source venv/bin/activate && pytest` → 231 hermetic tests (units, loop behavior,
